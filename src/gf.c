...
JL_DLLEXPORT void jl_specialization_set_world(jl_lambda_info_t *li, size_t min_world, size_t max_world)
{
    if (li->def == NULL)
        return;
...
static jl_lambda_info_t *jl_mt_assoc_by_type(jl_methtable_t *mt, jl_datatype_t *tt, int cache, int inexact, int allow_exec, size_t world)
{
    // caller must hold the mt->writelock
...
static void invalidate_recursive(jl_array_t *backedges, size_t max_world)
{
    if (backedges) {
        ...
        for (i = 0; i < l; i++) {
            ...
            env.invalidated = 0;
            env.replaced = (jl_lambda_info_t*)jl_array_ptr_ref(backedges, i);
...
static int invalidate_conflicting(jl_typemap_entry_t *oldentry, struct typemap_intersection_env *closure0)
{
    ...
    for (i = 0; i < n; i++) {
        if (d[i] == oldentry->func.linfo->def) {
            ...
            jl_lambda_info_t *li = oldentry->func.linfo;
                                           ...
                                           int cache, int inexact, int allow_exec, size_t world)
{
    ...
    if (jl_is_leaf_type((jl_value_t*)types))
        cache = 1;
    jl_lambda_info_t *sf = jl_mt_assoc_by_type(mt, types, cache, inexact, allow_exec, world);
    if (cache) {
        JL_UNLOCK(&mt->writelock);
    }
    else {
...
jl_lambda_info_t *jl_get_specialization1(jl_tupletype_t *types, size_t world)
{
    ...
    JL_GC_PUSH1(&sf);
    JL_TRY {
        sf = jl_method_lookup_by_type(mt, types, 1, 1, 1, world);
    } JL_CATCH {
        sf = NULL;
    }
...
{
    jl_lambda_info_t *li = jl_get_specialization1(types, world);
...
static void _compile_all_deq(jl_array_t *found)
{
    ...
    for (found_i = 0; found_i < found_l; found_i++) {
        ...
        if (m->isstaged)
            ...
            linfo = jl_specializations_get_linfo(m, ml->sig, jl_emptysvec, jl_world_counter);
